...
public class DocumentProcessor {
    ...
    private void resetChain() {
        _fileValidationChain = new BasicDocumentValidationStep(_repository);
        _context.resetFileInfo(); // Reset lại thông tin file trong context để tránh lỗi khi nhập lại thông tin cá
                                  // nhân sau khi đã nhập tệp đính kèm trước đó
    }
    ...
    public boolean proccessInsertPersonalInfo(Document doc) {
        ...
        _context = new FileValidationContext(doc.applicantName, doc.applicantEmail, doc.applicantPhone); // Tạo context
                                                                                                         // với thông
                                                                                                         // tin người
                                                                                                         // nộp để
                                                                                                         // truyền vào
        ...
        // Nếu đang ở trạng thái "Bản nháp"/"Đã tải file" do nhập lại thông tin cá nhân
        // sau khi đã vào trạng thái khác, reset lại chuỗi kiểm tra để đảm bảo các bước
        // kiểm tra được thực hiện đầy đủ khi nhập lại thông tin cá nhân
        if (doc.status == DocumentStatus.BAN_NHAP || doc.status == DocumentStatus.DA_TAI_FILE) {
            resetChain(); // Nếu đang ở trạng thái "Bản nháp", reset lại chuỗi kiểm tra để đảm bảo các
                          // bước kiểm tra được thực hiện đầy đủ khi nhập lại thông tin cá nhân
        }
        // Thực hiện chuỗi kiểm tra thông tin cá nhân ngay sau khi nhập, nếu không hợp
        // lệ sẽ trả về false và dừng quy trình tiếp nhận
    ...
    public boolean proccessInsertDocumentFile(Document doc) {
        ...
        if (doc.extractedContent == null || doc.extractedContent.isBlank()) {
            ...
            try {
                DocumentExtension docExtension = doc.fileExtension;
        ...
        _context.setFileInfo(doc.documentType.toString(), doc.filePath, doc.fileExtension.toString(), doc.fileSizeKB,
                doc.digitalSignature, doc.extractedContent);

        // Tạo chuỗi kiểm tra (giữ lại step đầu làm "head" của chain)
        _fileValidationChain.setNext(new BasicFileValidationStep(_repository))
    ...
    public void proccessInsertSubmissionInfo(Document doc) {
        ...
        resetChain(); // Sau khi hoàn thành quy trình tiếp nhận, reset lại chuỗi kiểm tra để sẵn sàng
                      // cho hồ sơ tiếp theo
